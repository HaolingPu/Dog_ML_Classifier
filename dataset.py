...
class ImageStandardizer(object):
    ...
    def fit(self, X):
        ...
        X = X.astype(np.float64)  # Ensure the input X is in float64 format
        
        # self.image_mean, is an array of shape (3,), so self.image_mean[0] is the mean of all red channel values across all images.
        self.image_mean = X.mean(axis=(0, 1, 2))  # mean for each channel (R, G, B)
