...
def main():
    ...
    model = ViT(
        num_patches=16,        # Number of patches along each dimension
        num_blocks=2,          # Number of transformer encoder blocks
        num_hidden=16,         # Embedding dimension (hidden dimension)
        num_heads=2,           # Number of attention heads
        num_classes=2,         # Output classes for classification
        chw_shape=(3, 64, 64) 
    )
