...
def early_stopping(stats, curr_count_to_patience, prev_val_loss):
    ...
    if current_val_loss < prev_val_loss:
        # If improved, reset count and update previous validation loss
...
def predictions(logits):
    ...
    predicted_classes = torch.argmax(logits, dim=1) # Dim 1 corresponds to the class dimension (num_classes)
